...
export function AttendanceEmployeeView() {
  ...
  // Fetch projects (employee now has access to employer's projects via RLS)
  ...
  const checkInMutation = useMutation({
    mutationFn: async (projectId: string) => {
      ...
      const { error } = await supabase.from("attendance_records").insert({
        ...
        employer_id: project.user_id, // Use project owner as employer
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Checka in och ut från arbetsplatsen
          </p>
        </div>
      </div>

      {/* Simple Check In/Out Card */}
